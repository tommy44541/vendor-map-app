...
const LocationPage = () => {
  ...
  const listRef = useRef<FlatList<MerchantLocation>>(null);
  ...
  const selectLocation = (location: MerchantLocation, index?: number) => {
  ...
  const loadSavedLocations = async (
    ...
    try {
      ...
      const response = await merchantApi.getMerchantLocations();
      if (response.success) {
  ...
  const saveLocation = async () => {
    ...
    try {
      setIsLoading(true);
      const locationData: CreateMerchantLocationRequest = {
        label: nextLabel,
      ...
      const response = await merchantApi.createMerchantLocation(locationData);
      if (response.success) {
  ...
  const openEditModal = (location: MerchantLocation) => {
  ...
  const submitEdit = async () => {
    ...
    try {
      ...
      const response = await merchantApi.updateMerchantLocation(
        editLocationId,
        {
          label: nextLabel,
  ...
  const confirmDelete = (location: MerchantLocation) => {
    Alert.alert("確認刪除", `確定要刪除「${location.Label}」嗎？`, [
      { text: "取消", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            setIsListLoading(true);
            const response = await merchantApi.deleteMerchantLocation(
              location.ID
            );
            if (response.success) {
